...
void decode(struct fft_collection* coll, char *the_string){
  ...
  int samp, samp_per_bit = coll->N/wave_packets;
  int by, bytes_in_data = wave_packets/8;
  int samp_per_byte = samp_per_bit*8;
  double max = 0.0, limit;
  char acc;

  // go through half the data to find the max
  for(bi=0;bi<wave_packets/2;bi++){
    for(samp=0;samp<samp_per_bit;samp++){
      if(fabs(REAL(coll->data, (samp_per_bit*bi)+samp)) > max)
        max = fabs(REAL(coll->data, (samp_per_bit*bi)+samp));
    }
  }
  ...
  for(by=0;by<bytes_in_data;by++){ // Go through each byte
    ...
    for(bi=0;bi<8;bi++){           // Go through each bit
      for(samp=0;samp<samp_per_bit;samp++){ // Loop over samples in one bit
        // Test if we found 1-bit
        if(fabs(REAL(coll->data,(samp_per_byte*by)+(samp_per_bit*bi)+samp)) > limit){
          acc |= 1 << (7-bi);      // Put bit into acc in correct position
          break;
        }
      }
    }
